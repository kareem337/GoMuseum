...
class build_generator(nn.Module):
    ...
    def estimate_meanvar(self,x,mask_,eps):
        x_cnt = torch.max(torch.FloatTensor([eps]),
...
class SemanticRegenerationNet(nn.Module):
    ...
    def relative_spatial_variant_mask(self,mask,hsize=64,sigma=1/40,iters=9):
        ...
        kernel = make_gauss_var(hsize,sigma)
    ...
    def gradients_penalty(self,x,y,mask=None,norm=1):
        ...
        gradients = torch.autograd.grad(y,x,create_graph=True,grad_outputs=torch.ones(y.size()),retain_graph=True,only_inputs=True)[0]
        if mask is None:
    ...
    def forward(self,x,oG=None,oD=None):
        ...
        mask = torch.tensor(1 - mask,requires_grad=False)
        h,w = x.shape[2:]
...
if __name__ == "__main__":
    #with torch.no_grad():
    testin = Variable(torch.randn(2,3,256,256),requires_grad=True)
    srnet = SemanticRegenerationNet(args)
    #srnet.train()
